...
def get_SE_CI_antithetic(S0, K, r, q, sigma, T, N):
    # We generate N pairs, for a total of 2N paths
    z = np.random.standard_normal(N)
    
    # Common terms for both paths
    drift = (r - q - 0.5 * sigma**2) * T
    vol_sqrt_t = sigma * np.sqrt(T)
    discount = np.exp(-r * T)
    
    # Generate the two terminal stock price sets
    ST1 = S0 * np.exp(drift + vol_sqrt_t * z)
    ST2 = S0 * np.exp(drift + vol_sqrt_t * (-z))
    
    # Calculate discounted payoffs (for a Put option)
    payoff1 = np.maximum(K - ST1, 0) * discount
    payoff2 = np.maximum(K - ST2, 0) * discount
    
    # Average the pairs to create N "antithetic observations"
    # This is where the variance reduction happens!
    antithetic_averages = (payoff1 + payoff2) / 2.0
    
    # Calculate Final Statistics
    estimate_price = np.mean(antithetic_averages)
    
    # SE is the standard deviation of the averaged pairs divided by sqrt(N)
